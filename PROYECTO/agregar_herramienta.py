...
def cargar_herramientas():
    try:
        ...
        registrar_log(f"Archivo {ARCHIVO_HERRAMIENTAS} no encontrado, creando nuevo", "WARNING")
        return []
    except json.JSONDecodeError as e:
        registrar_log(f"Error al decodificar {ARCHIVO_HERRAMIENTAS}: {e}", "ERROR")
        return []
...
def guardar_herramientas(herramientas):
    try:
        ...
        registrar_log(f"Error al guardar herramientas: {e}", "ERROR")
...
def agregar_herramienta():
    ...
    if any(str(h["id"]) == id_h for h in herramientas):
        print(Fore.RED + "   Ya existe una herramienta con ese ID.")
        registrar_log(f"Intento de agregar herramienta con ID duplicado: {id_h}", "WARNING")
        pausa()
        return

    nombre = input("   ▸ Nombre: ").strip().title()
    categoria = input("   ▸ Categoría: ").strip().lower()
    estado = input("   ▸ Estado (activa, en reparación, fuera de servicio): ").strip().lower()

    if not nombre or not categoria or estado not in ESTADOS_VALIDOS:
        print(Fore.RED + "   Datos inválidos o estado no permitido.")
        registrar_log(f"Datos inválidos al agregar herramienta - Nombre: {nombre}, Estado: {estado}", "WARNING")
        pausa()
        return
    ...
    registrar_log(f"Herramienta agregada: {nombre} (ID: {id_h}, Cantidad: {cantidad})", "INFO")
    pausa()
...
def listar_herramientas():
   ...
   ▸ ID: {h['id']}
     ...
     Cantidad: {h['cantidad']}
     Estado: {color_estado}{h['estado']}{Fore.WHITE}
     Valor: ${h['valor']:,}
   {"─" * 50}
""")
    pausa()
...
def buscar_herramienta():
    ...
    encontrada = False
    for h in herramientas:
        if nombre_buscar in h["nombre"].lower():  # Búsqueda parcial
            if not encontrada:
...
""")
    ...
    if not encontrada:
        ...
        registrar_log(f"Búsqueda sin resultados: {nombre_buscar}", "INFO")
...
def actualizar_herramienta():
    ...
    for h in herramientas:
        if str(h["id"]) == id_buscar:
            ...
            nuevo_nombre = input(f"   ▸ Nuevo nombre ({h['nombre']}): ").strip()
            nueva_categoria = input(f"   ▸ Nueva categoría ({h['categoria']}): ").strip()
            nueva_cantidad = input(f"   ▸ Nueva cantidad ({h['cantidad']}): ").strip()
            nuevo_estado = input(f"   ▸ Nuevo estado ({h['estado']}): ").strip().lower()
            nuevo_valor = input(f"   ▸ Nuevo valor ({h['valor']}): ").strip()
            ...
            if nueva_categoria:
                ...
                cambios.append(f"categoría: {nueva_categoria}")
            ...
            if nueva_cantidad:
                try:
                    ...
                    if cantidad_int >= 0:
                        ...
                        cambios.append(f"cantidad: {cantidad_int}")
                except ValueError:
                    print(Fore.RED + "   Cantidad inválida.")
            ...
            if nuevo_valor:
                try:
                    ...
                    if valor_int >= 0:
                        h["valor"] = valor_int
                        cambios.append(f"valor: {valor_int}")
                except ValueError:
                    print(Fore.RED + "   Valor inválido.")
            ...
            if cambios:
                registrar_log(f"Herramienta {id_buscar} actualizada: {', '.join(cambios)}", "INFO")
            
            pausa()
            return

    print(Fore.RED + "   Herramienta no encontrada.")
    registrar_log(f"Intento de actualizar herramienta inexistente: {id_buscar}", "WARNING")
    pausa()
...
def eliminar_herramienta():
    ...
    for h in herramientas:
        if str(h["id"]) == id_eliminar:
            ...
            if confirmacion == "s":
                ...
                registrar_log(f"Herramienta eliminada: {nombre_eliminado} (ID: {id_eliminar})", "INFO")
            else:
    ...
    print(Fore.RED + "   Herramienta no encontrada.")
    registrar_log(f"Intento de eliminar herramienta inexistente: {id_eliminar}", "WARNING")
    pausa()
...
def menu_herramientas():
    while True:
        titulo("GESTIÓN DE HERRAMIENTAS")
